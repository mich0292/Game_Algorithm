...
namespace Project
{
    public class Boss : GameObject
    {
        ...
        {           
            ...
            /*
            foreach (GameObject bullet in Game1.playerBulletList)
            {
                float futureTime = (float)gameTime.ElapsedGameTime.TotalSeconds + 0.00001f;
                Console.WriteLine((float)gameTime.ElapsedGameTime.TotalSeconds);
                Vector2 futurePos = bullet.position * bullet.speed * futureTime;
                if (BoundingBox.Intersects(new Rectangle((int)futurePos.X, (int)futurePos.Y, bullet.texture.Width, bullet.texture.Height)))
                {
